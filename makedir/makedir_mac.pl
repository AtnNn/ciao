% ===========================================================================
:- module(makedir_mac, _, [make, fsyntax, assertions]).
% ===========================================================================
:- doc(title, "Mac packages generator").
...
installer_pkg :-
	...
	package_pkg(DestDir, TmpDir, PackDir, WorkSpace, 'Ciao',
	    ~componentversion(ciaode), PkgName),
...
'/Developer/Applications/Utilities/PackageMaker.app/Contents/MacOS/PackageMaker',
	PkgName = ~atom_concat([PkgPath, '/', ~version(~wholesystem), '.pkg']),
...
package_dmg(PkgPath, List) :-
	...
	DmgName = ~atom_concat([PkgPath, '/', ~version(~wholesystem), '.dmg']),
	bold_message("Wrapping the package into a dmg image"),
	process_list_for_src(List, Tail),
	do(['hdiutil create ', DmgName, ' -volname ', ~version(~wholesystem)
		|Tail], []).
...
homeURL("http://www.clip.dia.fi.upm.es/Software/Ciao/Beta").
packageDir("/packages/CiaoDE/").
...
generate_portfile(SkelFileName, DestFileName) :- PackageNameVersion =
	~atom_codes(~version(~wholesystem)),
	replace_strings_in_file([["<v>Version</v>", ~atom_codes(
			~componentversion(ciaode))], [
		    "<v>PackageNameVersion</v>", PackageNameVersion], [
		    "<v>HomeURL</v>", ~homeURL], ["<v>MasterURL</v>",
		    ~append(~append(~homeURL, ~packageDir), PackageNameVersion)
		],
...
installer_app :-
	...
	lpmake_subdir(ciao, makedir_part_ciao, emacs_support),
	del_name_value(emacs_type),


	replace_strings_in_file(
	    [["<VERSION>", ~atom_codes(~version(~wholesystem))],
	...
	replace_strings_in_file(
	    [["<VERSION>", ~atom_codes(~version(~wholesystem))],
		["<SORTVERSION>", ~atom_codes(~componentversion(ciaode))],
		["<DOMAIN>",      Domain]],
