:- module(clp_dump, [dump_internal/3], [dcg,.(ops),.(clp_src)]).
%
...
%
tidy_frozen_system_op('nl_eval:solve_abs'(_,A,B,_), Dict) -->
  tidy_equation( A, abs(B), Dict).
tidy_frozen_system_op('nl_eval:solve_mix'(_,Mix,A,B,C,_), Dict) -->
  { F =.. [Mix,B,C] },
  tidy_equation( A, F, Dict).
tidy_frozen_system_op('nl_eval:solve_mult'(_,A,B,C,_), Dict) -->
  tidy_equation( A, B*C, Dict).
tidy_frozen_system_op('nl_eval:solve_pow'(_,A,B,C,_), Dict) -->
  tidy_equation( A, pow(B,C), Dict).
tidy_frozen_system_op('nl_eval:solve_trig'(_,Trig,A,B,_), Dict) -->
  { F =.. [Trig,B] },
  tidy_equation( A, F, Dict).
