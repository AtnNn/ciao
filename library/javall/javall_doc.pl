...
:- comment(title,"Low-level Prolog/Java Bidirectional Interface").

:- comment(subtitle,"@bf{The CIAO System Documentation Series}").
%:- comment(subtitle,"Technical Report CLIP 5/97.1").
:- comment(subtitle,"@em{Printed on:} @today{}").

:- comment(author, "Jes@'{u}s Correas Fern@'{a}ndez and The CLIP Group").
...
:- comment(summary,"@include{JavallSumm.lpdoc}").
...
bidirectional Java/Prolog low-level interface. The Java side of this
