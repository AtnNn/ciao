...
:- comment(title,"Measuring features from predicates (time cost or memory used...)").
...
# "performance accepts a goal, @var{P}, as a first argument. The aim of
  this predicate is to call @var{P} several times and meassure some feature
  (in this version, only time, that is because no extra parameter has been
  added). @var{M} defines how many times @var{P} should be called. Usually,
  calling the predicate in some succesion (10,100,1000) and dividing by the
  number of times it is executed we can obtain the \"execution time\" of
  the predicate (if we are measuring time).
...
performance( Pred , graph(S,E,I) , TimesEnd ) :- 
	!,
	exec_graph_pred(Pred,       S , E , I , Times  ),
	exec_graph_pred(do_nothing, S , E , I , Times2 ),
...
# "@var{A} is a list of pairs (P1,P2). @var{B} is a list of pairs with the form (P1,P2/P1) for each (P1,P2) that belongs to @var{A}".
...
:- comment(average_mode(Flag), "@var{Flag} controls the treatement of the
...
benchmark( Pred , Mod , BCH_LIST , Method , NumTimes , _ , Times ) :-
	...
	ibenchmark( ModPred , BCH_LIST , Method , NumTimes , _ , Times ).


ibenchmark( Pred , [(Arg1,XPos) | BCH_LIST] , Method , NumTimes , _ , [ (XPos,Med) | RTimes ] ) :-
	T =.. [Pred,Arg1],
	term_to_meta( T , MT ) ,
        module_concat(time_analyzer, measure_time, TimePred),
	time_of_bench_ntimes( TimePred , MT , NumTimes , Time  ), !,
	time_of_bench_ntimes( TimePred , do_nothing   , NumTimes , Time2 ),
	compute_average( Time2 , average , NothingTime ),
	map( Time , (''(A,B):- B is A - NothingTime) , RealTime ),
	compute_average( RealTime , Method , Med ),
	ibenchmark( Pred , BCH_LIST , Method , NumTimes , _ , RTimes ).
...
:- pred compare_benchmark( ListPred , BenchList , Method , NumTimes , BaseName , Reserved , GeneralOptions ) 
	: list(pred) * list * average_mode * int * atom * term * var => 
	  list(pred) * list * average_mode * int * atom * term * list
...
compare_benchmark( Preds , Mod , Bench , Method , NumTimes , BaseName , _ , GeneralOptions ) :-
	generate_benchmark_list( Preds , Mod , Bench , Method , NumTimes , _ , Times ),
	generate_plot( BaseName , Times , GeneralOptions ).



generate_benchmark_list( [(P1,LO1)|R] , Mod , Bench , Method , NumTimes , _ , [(Times1,LO1)|TR] ) :-
        module_concat(Mod, P1, NewP),!,
	ibenchmark( NewP , Bench , Method , NumTimes , _  , Times1 ), !,
	generate_benchmark_list( R , Mod , Bench , Method , NumTimes , _  , TR ).

generate_benchmark_list( [(P1,LO1,Module)|R] , Mod , Bench , Method , NumTimes , _ , [(Times1,LO1)|TR] ) :-
        module_concat(Module, P1, NewP),!,
	ibenchmark( NewP , Bench , NumTimes , Method , _ , Times1 ), !,
	generate_benchmark_list( R , Mod , Bench , Method , NumTimes , _ , TR ).

generate_benchmark_list( [function(A)|R] , Mod , Bench , Method , NumTimes , _ , [function(A)|TR] ) :-
	! , 
	generate_benchmark_list( R , Mod , Bench , Method , NumTimes , _ , TR ).

generate_benchmark_list( [ P1|R] , Mod , Bench , Method , NumTimes , _ , [(Times1,[])|TR] ) :-
        module_concat(Mod, P1, NewP),!,
	ibenchmark( NewP , Bench , Method , NumTimes , _ , Times1 ), !,
	generate_benchmark_list( R , Mod , Bench , Method , NumTimes , _ , TR ).
...
# "The predicate @var{P}, which accepts TWO arguments, is called
 ...
 predicate generate_plot. @var{What} is reserved for future use".
...
ibenchmark2( Pred , [(Arg1,XPos) | BCH_LIST] , Method , NumTimes , _ , [ (XPos,Med) | RTimes ] ) :-
	time_of_bench_ntimes( Pred, Arg1 , NumTimes , Time  ), !,
...
:- pred compare_benchmark2( ListPred , BenchList , Method , NumTimes , BaseName , Reserved , GeneralOptions ) 
	: list(pred) * list * average_mode * int * atom * atom * list => 
	  list(pred) * list * average_mode * int * atom * atom * list
...
compare_benchmark2( Preds , Mod , Bench , Method , NumTimes , BaseName , Opt , GeneralOptions ) :-
	generate_benchmark_list2( Preds , Mod , Bench , Method , NumTimes , Opt , Times ),
	generate_plot( BaseName , Times , GeneralOptions ).

generate_benchmark_list2( [(P1,LO1)|R] , Mod , Bench , Method , NumTimes , Opt , [(Times1,LO1)|TR] ) :-
        module_concat(Mod, P1, NewP),
	ibenchmark2( NewP , Bench , Method , NumTimes , Opt , Times1 ), !,
	generate_benchmark_list2( R , Mod , Bench , Method , NumTimes , Opt , TR ).

generate_benchmark_list2( [(P1,LO1,Module)|R] , Mod , Bench , Method , NumTimes , Opt , [(Times1,LO1)|TR] ) :-
        module_concat(Module, P1, NewP),
	ibenchmark2( NewP , Bench , Method , NumTimes , Opt , Times1 ), !,
	generate_benchmark_list2( R , Mod , Bench , Method , NumTimes , Opt , TR ).

generate_benchmark_list2( [function(A)|R] , Mod , Bench , Method , NumTimes , Opt , [function(A)|TR] ) :-
	! , 
	generate_benchmark_list2( R , Mod , Bench , Method , NumTimes , Opt , TR ).

generate_benchmark_list2( [ P1|R] , Mod , Bench , Method , NumTimes , Opt , [(Times1,[])|TR] ) :-
        module_concat(Mod, P1, NewP),
	ibenchmark2( NewP , Bench , Method  , NumTimes , Opt , Times1 ), !,
	generate_benchmark_list2( R , Mod , Bench , Method , NumTimes , Opt , TR ).
...
% POR HACER:
% poner los ejemplos en la documentacion
:- comment(version_maintenance,dir('../../version')).

:- comment(version(1*0+2,2003/12/03,00:10*32+'CET'), "documentation errors
   corrected and improved a bit (David Trallero Mena)").

:- comment(version(1*0+1,2003/12/01,23:07*39+'CET'), "benchmark2/6 was
   documented as benchmark2/6 (David Trallero Mena)").

:- comment(version(1*0+0,2003/12/01,22:27*16+'CET'), "Version 1.0 (David
   Trallero Mena)").
