...
prec([(Pred_desc, Precond, Postcond)|RS],Goal,NRS):-
	instance(Goal,Pred_desc),
	entailed(Precond,Pred_desc,Goal),!,
...
disentailed(Prop,Pred_desc,Goal):-
	\+(\+((Pred_desc=Goal, disentailed(Prop)))).
...
entailed(Prop,Pred_desc,Goal):-
	\+(\+((Pred_desc=Goal, is_entailed(Prop)))).
	
is_entailed((Prop,Props)):-!,
	is_entailed(Prop),
	is_entailed(Props).
is_entailed((Prop;Props)):-!,
 	(is_entailed(Prop);is_entailed(Props)).
...
pred_ass_possibly_entailed((Pred_desc, Prec, _Postcond),Goal):-
	 instance(Goal,Pred_desc),disentailed(Prec,Pred_desc,Goal),!,fail. 
