...
:- pred retract_fact(Fact) : callable
# "Same as @pred{pretractall/1}, but if the predicate concerned is not
   persistent then behaves as the builtin of the same name.  Defined in the
   @lib{persdb} package.".
...
make_operation(ra(Pred), Terms, Terms_, NTerms, Terms_) :-
	(var(Terms) ->
	 NTerms = Terms
	;delete_non_ground(Terms, Pred, NTerms)
	), !.
...
:- comment(version(1*11+213,2004/03/25,11:53*27+'CET'), "Fixed a bug
   in make_operation/5 which raised incorrect process of
   retractall_fact directives.  (Jose Manuel Gomez Perez)").

:- comment(version(1*11+212,2004/03/24,16:43*01+'CET'),
