...
:- comment(title,"Unweighted graph-processing utilities").

:- comment(author,"M. Carlsson, adapted from shared code written by
   Richard A O'Keefe. Mods by F.Bueno and M.Carro.").

:- comment(module,"An unweighted directed graph (ugraph) is
...
:- comment(version(0*9+105,1999/06/04,12:24*49+'MEST'), "New
   ...
   comments added, sorry.  Will improve this in the future.  (MCL)").

:- comment(version(0*4+5,1998/2/24), "Synchronized file versions with
   global Ciao version.  (Manuel Hermenegildo)").

:- comment(version(0*0+0,1997/12/18), "A first shot: only
   vertices_edges_to_ugraph/3, original from SICStus. (Francisco Bueno
   Carrillo)").
