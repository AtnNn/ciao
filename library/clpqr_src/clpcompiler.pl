...
% clp(r) compiler
%
:- module(clpcompiler, [compile_constr/4],[dcg,.(clp_src)]).
...
% Check this!
ctidy_nonlin( 'nl_eval:solve_abs'(_,A,B,_))       --> [ solve_abs(A,B) ].
ctidy_nonlin( 'nl_eval:solve_mix'(_,Mix,A,B,C,_)) --> [ solve_mix(Mix,A,B,C) ].
ctidy_nonlin( 'nl_eval:solve_mult'(_,A,B,C,_))    --> [ solve_mult(A,B,C) ].
ctidy_nonlin( 'nl_eval:solve_pow'(_,A,B,C,_))     --> [ solve_pow(A,B,C) ].
ctidy_nonlin( 'nl_eval:solve_trig'(_,Trig,A,B,_)) --> [ solve_trig(Trig,A,B) ].
