...
try_assertion(assertion(Status, Body, VarNames, Loc)) := PredDescription :-
	...
	( Kind = foreign ->
        ...
        ; Kind = native ->
	    PredDescription = native(PredName, ForeignName)
	; fail
	).
...
get_name_and_kind_2(DefaultName, GP) := (DefaultName, native, GP1) :- select(native(_), GP, GP1), !.
get_name_and_kind_2(_, GP) := (ForeignName, foreign, GP1) :- select(foreign(_, ForeignName), GP, GP1), !.
get_name_and_kind_2(_, GP) := (ForeignName, native, GP1) :- select(native(_, ForeignName), GP, GP1), !.
...
no_more_foreign_name(Loc, PredName, _) :-
	error_message(Loc, "more than one foreign/1,  foreign/2,  native/1 or native/2 property in predicate ~w", [PredName]), 
	set_fact(foreign_predicate_error), 
	fail.

valid_foreign_name(_, _, Name) :- atom(Name),  !.
valid_foreign_name(Loc, PredName, _) :-
	error_message(Loc, "invalid foreign/native function name in predicate ~w", [PredName]), 
        set_fact(foreign_predicate_error), 
	fail.
...
include_base_dir := ~atom_concat([~ciaolibdir, '/include/', ~get_os_arch]).
...
foreign_predicate_interface(P) --> { P = native(_, NativeName) }, !,
...
native_prototype(NativeName) --> [NativeName:function([pointer(struct(worker))], 'BOOL')].
