...
:- comment(version(1*5+107,2000/04/05,09:55*16+'CEST'), "When debugging,
   ...
   getting a error message.  (Manuel Carlos Rodriguez)").

:- comment(version(1*5+103,2000/04/02,23:22*37+'CEST'), "All source
   debugger output is redirected to @tt{user} (was being sent to
   current output).  (Manuel Hermenegildo)").

:- comment(version(1*5+97,2000/03/30,11:17*28+'CEST'), "Update
   documentation.  (Manuel Carlos Rodriguez)").

:- comment(version(1*5+88,2000/03/24,13:18*46+'CET'), "Changed to
   'Ciao' in debugger prompts.  (Manuel Hermenegildo)").

:- comment(version(1*5+58,2000/02/28,10:32*04+'CET'), "When reaching a
   breakpoint the debugger shows the line with + indicating either a
   breakpoint or a spypoint.  (Manuel Carlos Rodriguez)").

:- comment(version(1*5+19,1999/12/16,17:55*37+'MET'), "Changed
   @pred{write_canonical/1} in debugger when attributed variables ocurr to
   @pred{write_term} with max_depth.  (Daniel Cabeza Gras)").
...
:- comment(version(1*5+17,1999/12/14,22:28*52+'MET'), "Fixed bug in the
   ...
   such when printing debug information.  Also changed @tt{toplev.pl}.
   (Daniel Cabeza Gras)").
...
:- comment(version(1*3+104,1999/11/17,22:05*53+'MET'), "Fixed a bug when
   calling @pred{debugging/0} by inputing '=' at the trace prompt. (Daniel
   Cabeza Gras)").

:- comment(version(0*6+3,1998/07/20,18:51*26+'MET DST'), "Added
   @pred{debug_module/1} and @pred{nodebug_module/1} to replace
   @pred{debug_modules/1}.  (Daniel Cabeza Gras)").
