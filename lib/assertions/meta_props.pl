...
:- comment(title,"Meta-properties").
:- comment(author,"Francisco Bueno").
:- comment(module,"This library allows the use of some meta-constructs
...
:- comment(usage,"@tt{:- use_module(library('assertions/meta_props'))}

   or also as a package @tt{:- use_package(metaprops)}.

   Note the different names of the library and the package.").

:- comment(bug,"Using a hook predicate is not very elegant. Need something
        else.").
:- comment(bug,"The cut in the hook prevents backtracking (enough for most
...
:- comment(doinclude,prop_abs/1).

:- comment(prop_abs(Prop),
...
:- comment(callme/2,"(User defined.) A hook predicate you have to define
