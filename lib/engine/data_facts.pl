...
:- comment(module,"@cindex{data predicate}Prolog implementations
    traditionally implement the concept of @concept{dynamic predicate}s:
    predicates which can be inspected or modified at run-time, adding or
    deleting individual clauses.  The powerfulness of this feature
    brings also a great disadvantage: as new clause bodies can be
    arbitrarily added to the program, new unknown predicate calls can
    arise, thus preventing any global analysis or optimization of the
    ...
    in terms of speed, but above all turns the code automatically
