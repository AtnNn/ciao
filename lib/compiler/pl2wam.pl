...
straight_body([pcall(S, Size)|Gs], List,
	...
	{structure(S, 'basiccontrol:;'(G1,G2)), !,
	 internal_name(SubName), % jf-new solve multifile bug
...
internal_name(SubName) :- !,
	...
	atom_concat(Module, ':$$$', X0),
	atom_concat(X0, CounterAtom, SubName).
...
name_of_function(truncate(X), 4, X).
