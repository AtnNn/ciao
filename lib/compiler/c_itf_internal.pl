...
read_record_file_(PlName, Base, Type) :-
        ...
        expand_module_decl(Sentence, Base, module(Module, Exports, Package), Rest),
	...
	do_include_package(prelude, Base, Module, Ln0, Ln1),
	( nonvar(Package), member(pure, Package) -> 
	    true
	; do_include_package(nonpure, Base, Module, Ln0, Ln1)
	),
        assert_export_list(Exports, Base, Ln0, Ln1),
        do_include_package(Package, Base, Module, Ln0, Ln1),
	( member(Sentence2, Rest) ; repeat, read_sentence(Stream, Sentence2) ),
...
process_sentence(Sentence, Base, Pl, Module) :-
	...
	process_expanded_data_list(DataList, Base, Module, VNs, Sings, Pl, Ln0, Ln1).
process_sentence(_, _, _, _).

process_end_of_file(Base, Pl, Module, Ln0, Ln1) :-
	expand_term_to_list(end_of_file, Module, [], Data0),
	append(DataList, [end_of_file], Data0), !,
	process_expanded_data_list(DataList, Base, Module, [], [], Pl, Ln0, Ln1).
process_end_of_file(_, _, _, _, _).
...
normalize_module_decl(module(Module, Exports), _, module(Module, Exports, Package)) :- !,
...
process_expanded_data((:- Decl), Base, M, VNs,_Sings, Pl, Ln0, Ln1) :- !,
	( is_any_known_decl(Decl, Base) ->
	    ...
	    ; error_in_lns(Ln0, Ln1, error, ['declaration processing failed ',~~(Decl)])
	    )
...
process_expanded_data((H :- B), Base,_M, VNs, Sings, Pl, Ln0, Ln1) :- !,
        functor(H, F, A),
        ( atom(F) -> true
        ; error_in_lns(Ln0, Ln1, error, ['illegal clause in ' , H , ' :- ' , B ]), fail
        ),
...
do_include_package(F0, Base, Module, Ln0, Ln1) :-
	( atom(F0) ->
	    F = library(F0)
	; functor(F0, _, 1) ->
	    F = F0
	; compiler_error(Ln0, Ln1, bad_package_file(F0)),
	  F = error
	),
	( F = error ->
	    true
...
do_include(File, Base, Module,_Ln0,_Ln1) :-
	...
	repeat,
	read_sentence(Stream, Sentence),
	( Sentence = end_of_file(_, _) ->
	    true
	; process_sentence(Sentence, Base, SourceFile, Module),
	  fail
	),
	!,
        close(Stream),
...
do_add_sentence_trans(M, P, Base, Ln0, Ln1) :-
        ( add_sentence_trans(M, P) ->
            ...
            expand_term_to_list(0, M, _, _) % For initialization
...
:- comment(version(1*11+207,2004/03/04,23:00*37+'CET'), "Changed the
   predicate check_if_reimported/4 in mexpand.pl to compare EM instead
   of IM, to avoid unnecesary warnings when the same predicate is
   imported through different reexport paths (Jose Morales)").

:- comment(version(1*11+45,2003/09/23,12:58*15+'CEST'), "Added and
   exported predicate set_ciaopp_expansion/1 to enable custom
   predicate expansions required by CiaoPP.  (Jose Morales)").

:- comment(version(1*11+13,2003/04/07,19:21*10+'CEST'), "Added support
   for primitive_meta_predicate declaration (it can not be handled
   with a compilation module without entering a endless loop).  (Jose
   Morales)").

:- comment(version(1*11+10,2003/04/07,18:55*46+'CEST'), "Use package
   prelude.pl automatically in each module and include nonpure.pl when
   pure.pl package is not specified. Removed the concept of builtin
   module. Module metadata declared in prelude.pl and prefixed with $
   ('$u'/2, etc.).  (Jose Morales)").
