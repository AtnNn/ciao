:- module(c_itf,
	 ...
	 set_ciaopp_expansion/1,
         ...
         use_mod/3, use_mod/4, static_base/1, static_module/1, module_loaded/4,
         make_po_file/1,
...
% process_files_from(File, Mode{in,in$,...,po,...}, Type{module,user,any},
...
handle_related_files(Base, Mode):-
        related_file(Base, File, Type),
          base_name(File, BFile),
          \+ current_fact(processed(BFile, Mode)),
            get_file_itf(BFile, Type),
            asserta_fact(process_too(Mode, BFile)),
            follow_reexports(BFile,[BFile]),
        fail.
handle_related_files(_, _).

related_file(Base, File, module) :- uses_file(Base, File).
related_file(Base, File, user)   :- adds(Base, File).
...
warn_if_mismatch(Mod, Type) :-
        ( Mod = user(_) ->
          ( Type==module ->
              error_in_lns(_, _, warning,
                ['This user file was used as a module',
                 ' (use ensure_loaded instead)'])
          ; true
          )
        ; % Mod \= user(_)
          ( Type==user ->
              error_in_lns(_, _, warning,
                ['This module was loaded with ensure_loaded',
                 ' (use use_module instead)'])
          ; true
          )
        ).
...
process_decl(ensure_loaded(File), Base,_M,_VNs,_Ln0,_Ln1) :- !,
        ...
        assertz_fact(imports_all_1(Base, File)). % Import if a module
...
do_include_package(F, Base, Module, Ln0, Ln1) :-
        ...
        ; assertz_fact(package(Base,P)),
          do_include(P, Base, Module, Ln0, Ln1)
        ).
...
use_mod(File, Imports, ByThisModule, Type) :-
...
:- comment(version(1*9+361,2004/07/17,20:37*55+'CEST'), "Changed
   behavior of declarations @decl{use_module/1} and
   @decl{ensure_loaded/1}.  Now they act similar but give a warning on
   bad use. (Daniel Cabeza Gras)").

:- comment(version(1*9+358,2004/07/14,14:41*25+'CEST'), "Now repeated
   use_packages in files are ignored.  (Daniel Cabeza Gras)").

:- comment(version(1*9+322,2004/03/04,22:59*18+'CET'), "Changed the
