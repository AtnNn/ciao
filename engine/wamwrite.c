/* DO NOT EDIT!!! This file is autogenerated */
w_inittrue:
case INITTRUE:
ComputeE;
for (t0 = P1-sizeof(TAGGED);
     t0 >= EToY0*sizeof(TAGGED);
     t0 -= sizeof(TAGGED)) {
  LoadSVA(Yb(t0));
}
goto firsttrue;
w_firsttrue_n:
case FIRSTTRUE_N:
for (i = SP1, P++; i>0; --i)
  PUT_YVOID;
firsttrue:
E->next_insn = w->next_insn;
E->frame = w->frame;
w->frame = E;
w->next_insn = Pplus2;
w->local_top = StackCharOffset(E,P1);
if (OffStacktop(E,Stack_Warn))
  SetEvent;
DISPATCH_W(1);
w_initcallq:
case INITCALLQ:
P++;
goto w_initcall;
w_initcall:
case INITCALL:
ComputeE;
for (t0 = *(P+BPTP)-sizeof(TAGGED);
     t0 >= EToY0*sizeof(TAGGED);
     t0 -= sizeof(TAGGED)) {
  LoadSVA(Yb(t0));
}
goto w_firstcall;
w_firstcall_nq:
case FIRSTCALL_NQ:
P++;
goto w_firstcall_n;
w_firstcall_8q:
case FIRSTCALL_8Q:
P++;
goto w_firstcall_8;
w_firstcall_7q:
case FIRSTCALL_7Q:
P++;
goto w_firstcall_7;
w_firstcall_6q:
case FIRSTCALL_6Q:
P++;
goto w_firstcall_6;
w_firstcall_5q:
case FIRSTCALL_5Q:
P++;
goto w_firstcall_5;
w_firstcall_4q:
case FIRSTCALL_4Q:
P++;
goto w_firstcall_4;
w_firstcall_3q:
case FIRSTCALL_3Q:
P++;
goto w_firstcall_3;
w_firstcall_2q:
case FIRSTCALL_2Q:
P++;
goto w_firstcall_2;
w_firstcall_1q:
case FIRSTCALL_1Q:
P++;
goto w_firstcall_1;
w_firstcallq:
case FIRSTCALLQ:
P++;
goto w_firstcall;
w_call_nq:
case CALL_NQ:
P++;
goto w_call_n;
w_call_8q:
case CALL_8Q:
P++;
goto w_call_8;
w_call_7q:
case CALL_7Q:
P++;
goto w_call_7;
w_call_6q:
case CALL_6Q:
P++;
goto w_call_6;
w_call_5q:
case CALL_5Q:
P++;
goto w_call_5;
w_call_4q:
case CALL_4Q:
P++;
goto w_call_4;
w_call_3q:
case CALL_3Q:
P++;
goto w_call_3;
w_call_2q:
case CALL_2Q:
P++;
goto w_call_2;
w_call_1q:
case CALL_1Q:
P++;
goto w_call_1;
w_callq:
case CALLQ:
P++;
goto w_call;
w_lastcall_nq:
case LASTCALL_NQ:
P++;
goto w_lastcall_n;
w_lastcall_8q:
case LASTCALL_8Q:
P++;
goto w_lastcall_8;
w_lastcall_7q:
case LASTCALL_7Q:
P++;
goto w_lastcall_7;
w_lastcall_6q:
case LASTCALL_6Q:
P++;
goto w_lastcall_6;
w_lastcall_5q:
case LASTCALL_5Q:
P++;
goto w_lastcall_5;
w_lastcall_4q:
case LASTCALL_4Q:
P++;
goto w_lastcall_4;
w_lastcall_3q:
case LASTCALL_3Q:
P++;
goto w_lastcall_3;
w_lastcall_2q:
case LASTCALL_2Q:
P++;
goto w_lastcall_2;
w_lastcall_1q:
case LASTCALL_1Q:
P++;
goto w_lastcall_1;
w_lastcallq:
case LASTCALLQ:
P++;
goto w_lastcall;
w_firstcall_n:
case FIRSTCALL_N:
for (i = SP1, P++; i>8; --i)
  PUT_YVOID;
goto w_firstcall_8;
w_firstcall_8:
case FIRSTCALL_8:
PUT_YVOID;
goto w_firstcall_7;
w_firstcall_7:
case FIRSTCALL_7:
PUT_YVOID;
goto w_firstcall_6;
w_firstcall_6:
case FIRSTCALL_6:
PUT_YVOID;
goto w_firstcall_5;
w_firstcall_5:
case FIRSTCALL_5:
PUT_YVOID;
goto w_firstcall_4;
w_firstcall_4:
case FIRSTCALL_4:
PUT_YVOID;
goto w_firstcall_3;
w_firstcall_3:
case FIRSTCALL_3:
PUT_YVOID;
goto w_firstcall_2;
w_firstcall_2:
case FIRSTCALL_2:
PUT_YVOID;
goto w_firstcall_1;
w_firstcall_1:
case FIRSTCALL_1:
PUT_YVOID;
goto w_firstcall;
w_firstcall:
case FIRSTCALL:
E->next_insn = w->next_insn;
E->frame = w->frame;
w->frame = E;
w->next_insn = P+BPTP+LOffset;
w->local_top = StackCharOffset(E,*(P+BPTP));
P = BP1;
if (OffStacktop(E,Stack_Warn))
  SetEvent;
goto enter_predicate;
w_call_n:
case CALL_N:
for (i = SP1, P++; i>8; --i) {
  t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(i-1),&Yb(t1+1));
  } else {
    RefStack(X(i-1),&Yb(t1));
  }
}
goto w_call_8;
w_call_8:
case CALL_8:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(7),&Yb(t1+1));
  } else {
    RefStack(X(7),&Yb(t1));
  }
goto w_call_7;
w_call_7:
case CALL_7:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(6),&Yb(t1+1));
  } else {
    RefStack(X(6),&Yb(t1));
  }
goto w_call_6;
w_call_6:
case CALL_6:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(5),&Yb(t1+1));
  } else {
    RefStack(X(5),&Yb(t1));
  }
goto w_call_5;
w_call_5:
case CALL_5:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(4),&Yb(t1+1));
  } else {
    RefStack(X(4),&Yb(t1));
  }
goto w_call_4;
w_call_4:
case CALL_4:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(3),&Yb(t1+1));
  } else {
    RefStack(X(3),&Yb(t1));
  }
goto w_call_3;
w_call_3:
case CALL_3:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(2),&Yb(t1+1));
  } else {
    RefStack(X(2),&Yb(t1));
  }
goto w_call_2;
w_call_2:
case CALL_2:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(1),&Yb(t1+1));
  } else {
    RefStack(X(1),&Yb(t1));
  }
goto w_call_1;
w_call_1:
case CALL_1:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(0),&Yb(t1+1));
  } else {
    RefStack(X(0),&Yb(t1));
  }
goto w_call;
w_call:
case CALL:
w->next_insn = P+BPTP+LOffset;
P = BP1;
goto enter_predicate;
w_lastcall_n:
case LASTCALL_N:
for (i = SP1, P++; i>8; --i) {
    t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(i-1),&Yb(t1+1));
  } else {
    RefStack(X(i-1),&Yb(t1));
  }
  }
goto w_lastcall_8;
w_lastcall_8:
case LASTCALL_8:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(7),&Yb(t1+1));
  } else {
    RefStack(X(7),&Yb(t1));
  }
goto w_lastcall_7;
w_lastcall_7:
case LASTCALL_7:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(6),&Yb(t1+1));
  } else {
    RefStack(X(6),&Yb(t1));
  }
goto w_lastcall_6;
w_lastcall_6:
case LASTCALL_6:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(5),&Yb(t1+1));
  } else {
    RefStack(X(5),&Yb(t1));
  }
goto w_lastcall_5;
w_lastcall_5:
case LASTCALL_5:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(4),&Yb(t1+1));
  } else {
    RefStack(X(4),&Yb(t1));
  }
goto w_lastcall_4;
w_lastcall_4:
case LASTCALL_4:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(3),&Yb(t1+1));
  } else {
    RefStack(X(3),&Yb(t1));
  }
goto w_lastcall_3;
w_lastcall_3:
case LASTCALL_3:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(2),&Yb(t1+1));
  } else {
    RefStack(X(2),&Yb(t1));
  }
goto w_lastcall_2;
w_lastcall_2:
case LASTCALL_2:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(1),&Yb(t1+1));
  } else {
    RefStack(X(1),&Yb(t1));
  }
goto w_lastcall_1;
w_lastcall_1:
case LASTCALL_1:
t1 = P1, P++;
  if (t1&1) {
    RefStackUnsafe(X(0),&Yb(t1+1));
  } else {
    RefStack(X(0),&Yb(t1));
  }
goto w_lastcall;
w_lastcall:
case LASTCALL:
DEALLOCATE;
goto w_execute;
w_execute:
case EXECUTE:
P = BP1;
goto enter_predicate;
w_executeq:
case EXECUTEQ:
P = BP2;
goto enter_predicate;
w_put_x_void:
case PUT_X_VOID:
LoadHVA(Xb(P1),H);
DISPATCH_W(1);
w_put_x_variable:
case PUT_X_VARIABLE:
Load2HVA(Xb(P1),Xb(P2),H);
DISPATCH_W(2);
w_put_xval_xval:
case PUT_XVAL_XVAL:
Xb(P1) = Xb(P2);
Xb(P3) = Xb(P4);
DISPATCH_W(4);
w_put_x_value:
case PUT_X_VALUE:
Xb(P1) = Xb(P2);
DISPATCH_W(2);
w_put_x_unsafe_value:
case PUT_X_UNSAFE_VALUE:
RefStackUnsafe(Xb(P1),&Xb(P2));
Xb(P2) = t0;
DISPATCH_W(2);
w_put_y_first_variable:
case PUT_Y_FIRST_VARIABLE:
ComputeE;
goto w_put_y_variable;
w_put_y_variable:
case PUT_Y_VARIABLE:
t0 = P2;
Load2SVA(Xb(P1),Yb(t0));
DISPATCH_W(2);
w_put_yfvar_yvar:
case PUT_YFVAR_YVAR:
ComputeE;
goto w_put_yvar_yvar;
w_put_yvar_yvar:
case PUT_YVAR_YVAR:
t0 = P2;
Load2SVA(Xb(P1),Yb(t0));
t0 = P4;
Load2SVA(Xb(P3),Yb(t0));
DISPATCH_W(4);
w_put_constantq:
case PUT_CONSTANTQ:
Xb(P2) = T3;
DISPATCH_W(2+BPTP);
w_put_constant:
case PUT_CONSTANT:
Xb(P1) = T2;
DISPATCH_W(1+BPTP);
w_put_nil:
case PUT_NIL:
Xb(P1) = atom_nil;
DISPATCH_W(1);
w_put_largeq:
case PUT_LARGEQ:
StoreH;
Xb(P2) = MakeLarge(Arg,Pplus3);
LoadH;
DISPATCH_W(2+LargeInsns(T3));
w_put_large:
case PUT_LARGE:
StoreH;
Xb(P1) = MakeLarge(Arg,Pplus2);
LoadH;
DISPATCH_W(1+LargeInsns(T2));
w_put_structureq:
case PUT_STRUCTUREQ:
Xb(P2) = Tag(STR,H);
HeapPush(H,T3);
DISPATCH_W(2+BPTP);
w_put_structure:
case PUT_STRUCTURE:
Xb(P1) = Tag(STR,H);
HeapPush(H,T2);
DISPATCH_W(1+BPTP);
w_put_list:
case PUT_LIST:
Xb(P1) = Tag(LST,H);
DISPATCH_W(1);
w_put_y_value:
case PUT_Y_VALUE:
RefStack(Xb(P1),&Yb(P2));
DISPATCH_W(2);
w_put_y_unsafe_value:
case PUT_Y_UNSAFE_VALUE:
RefStackUnsafe(Xb(P1),&Yb(P2));
DISPATCH_W(2);
w_put_yval_yval:
case PUT_YVAL_YVAL:
RefStack(Xb(P1),&Yb(P2));
RefStack(Xb(P3),&Yb(P4));
DISPATCH_W(4);
w_put_yval_yuval:
case PUT_YVAL_YUVAL:
RefStack(Xb(P1),&Yb(P2));
RefStackUnsafe(Xb(P3),&Yb(P4));
DISPATCH_W(4);
w_put_yuval_yval:
case PUT_YUVAL_YVAL:
RefStackUnsafe(Xb(P1),&Yb(P2));
RefStack(Xb(P3),&Yb(P4));
DISPATCH_W(4);
w_put_yuval_yuval:
case PUT_YUVAL_YUVAL:
RefStackUnsafe(Xb(P1),&Yb(P2));
RefStackUnsafe(Xb(P3),&Yb(P4));
DISPATCH_W(4);
w_get_x_value:
case GET_X_VALUE:
StoreH;
goto r_get_x_value;
w_get_y_first_value:
case GET_Y_FIRST_VALUE:
StoreH;
goto r_get_y_first_value;
w_get_y_value:
case GET_Y_VALUE:
StoreH;
goto r_get_y_value;
w_get_constantq:
case GET_CONSTANTQ:
P++;
goto w_get_constant;
w_get_constant:
case GET_CONSTANT:
StoreH;
goto r_get_constant;
w_get_largeq:
case GET_LARGEQ:
P++;
goto w_get_large;
w_get_large:
case GET_LARGE:
StoreH;
goto r_get_large;
w_get_structureq:
case GET_STRUCTUREQ:
P++;
goto w_get_structure;
w_get_structure:
case GET_STRUCTURE:
StoreH;
goto r_get_structure;
w_get_nil:
case GET_NIL:
StoreH;
goto r_get_nil;
w_get_list:
case GET_LIST:
StoreH;
goto r_get_list;
w_get_constant_neck_proceedq:
case GET_CONSTANT_NECK_PROCEEDQ:
P++;
goto w_get_constant_neck_proceed;
w_get_constant_neck_proceed:
case GET_CONSTANT_NECK_PROCEED:
StoreH;
goto r_get_constant_neck_proceed;
w_get_nil_neck_proceed:
case GET_NIL_NECK_PROCEED:
StoreH;
goto r_get_nil_neck_proceed;
w_cutb_x:
case CUTB_X:
StoreH;
goto r_cutb_x;
w_cutb_x_neck:
case CUTB_X_NECK:
StoreH;
goto r_cutb_x_neck;
w_cutb_neck:
case CUTB_NECK:
StoreH;
goto r_cutb_neck;
w_cutb_x_neck_proceed:
case CUTB_X_NECK_PROCEED:
StoreH;
goto r_cutb_x_neck_proceed;
w_cutb_neck_proceed:
case CUTB_NECK_PROCEED:
StoreH;
goto r_cutb_neck_proceed;
w_cute_x:
case CUTE_X:
StoreH;
goto r_cute_x;
w_cute_x_neck:
case CUTE_X_NECK:
StoreH;
goto r_cute_x_neck;
w_cute_neck:
case CUTE_NECK:
StoreH;
goto r_cute_neck;
w_cutf:
case CUTF:
StoreH;
goto r_cutf;
w_cutf_x:
case CUTF_X:
StoreH;
goto r_cutf_x;
w_cut_y:
case CUT_Y:
StoreH;
goto r_cut_y;
w_choice_x:
case CHOICE_X:
Xb(P1) = ChoiceToInt(w->next_node);
DISPATCH_W(1);
w_choice_yf:
case CHOICE_YF:
ComputeE;
goto w_choice_y;
w_choice_y:
case CHOICE_Y:
Yb(P1) = ChoiceToInt(w->next_node);
DISPATCH_W(1);
w_kontinue:
case KONTINUE:
/* after wakeup, write mode! */Setfunc(TagToFunctor(Y(0)));
for(i=0; i<Func->arity; i++) X(i) = Y(i+1);
DEALLOCATE;
goto enter_predicate;
w_leave:
case LEAVE:
StoreH;
goto r_leave;
w_exit_toplevel:
case EXIT_TOPLEVEL:
StoreH;
goto r_exit_toplevel;
w_retry_cq:
case RETRY_CQ:
StoreH;
goto r_retry_cq;
w_retry_c:
case RETRY_C:
StoreH;
goto r_retry_c;
w_get_structure_x0q:
case GET_STRUCTURE_X0Q:
P++;
goto w_get_structure_x0;
w_get_structure_x0:
case GET_STRUCTURE_X0:
t1 = Tag(STR,H);
if (TagIsHVAw(t0))
  BindHVA(t0,t1)
else if (t0 & TagBitSVA)
  BindSVA(t0,t1)
else
  { BindCVA(t0,t1); Wake; }
HeapPush(H,T1);
DISPATCH_W(0+BPTP);
w_get_large_x0q:
case GET_LARGE_X0Q:
P++;
goto w_get_large_x0;
w_get_large_x0:
case GET_LARGE_X0:
StoreH;
t1 = MakeLarge(Arg,Pplus1);
LoadH;
if (TagIsHVAw(t0))
  BindHVA(t0,t1)
else if (t0 & TagBitSVA)
  BindSVA(t0,t1)
else
  { BindCVA(t0,t1); Wake; }
DISPATCH_W(0+LargeInsns(T1));
w_get_constant_x0q:
case GET_CONSTANT_X0Q:
P++;
goto w_get_constant_x0;
w_get_constant_x0:
case GET_CONSTANT_X0:
if (TagIsHVAw(t0))
  BindHVA(t0,T1)
else if (t0 & TagBitSVA)
  BindSVA(t0,T1)
else
  { BindCVA(t0,T1); Wake; }
DISPATCH_W(0+BPTP);
w_get_nil_x0:
case GET_NIL_X0:
if (TagIsHVAw(t0))
  BindHVA(t0,atom_nil)
else if (t0 & TagBitSVA)
  BindSVA(t0,atom_nil)
else
  { BindCVA(t0,atom_nil); Wake; }
DISPATCH_W(0);
w_get_list_x0:
case GET_LIST_X0:
t1 = Tag(LST,H);
if (TagIsHVAw(t0))
  BindHVA(t0,t1)
else if (t0 & TagBitSVA)
  BindSVA(t0,t1)
else
  { BindCVA(t0,t1); Wake; }
DISPATCH_W(0);
w_get_xvar_xvar:
case GET_XVAR_XVAR:
Xb(P2) = Xb(P1);
Xb(P4) = Xb(P3);
DISPATCH_W(4);
w_get_x_variable:
case GET_X_VARIABLE:
Xb(P2) = Xb(P1);
DISPATCH_W(2);
w_get_y_first_variable:
case GET_Y_FIRST_VARIABLE:
ComputeE;
goto w_get_y_variable;
w_get_y_variable:
case GET_Y_VARIABLE:
Yb(P2) = Xb(P1);
DISPATCH_W(2);
w_get_yfvar_yvar:
case GET_YFVAR_YVAR:
ComputeE;
goto w_get_yvar_yvar;
w_get_yvar_yvar:
case GET_YVAR_YVAR:
Yb(P2) = Xb(P1);
Yb(P4) = Xb(P3);
DISPATCH_W(4);
w_branch:
case BRANCH:
P = Pdeep;
DISPATCH_W(0);
w_function_1q:
case FUNCTION_1Q:
StoreH;
goto r_function_1q;
w_function_1:
case FUNCTION_1:
StoreH;
goto r_function_1;
w_function_2q:
case FUNCTION_2Q:
StoreH;
goto r_function_2q;
w_function_2:
case FUNCTION_2:
StoreH;
goto r_function_2;
w_builtin_1q:
case BUILTIN_1Q:
StoreH;
goto r_builtin_1q;
w_builtin_1:
case BUILTIN_1:
StoreH;
goto r_builtin_1;
w_builtin_2q:
case BUILTIN_2Q:
StoreH;
goto r_builtin_2q;
w_builtin_2:
case BUILTIN_2:
StoreH;
goto r_builtin_2;
w_builtin_3q:
case BUILTIN_3Q:
StoreH;
goto r_builtin_3q;
w_builtin_3:
case BUILTIN_3:
StoreH;
goto r_builtin_3;
w_retry_instance:
case RETRY_INSTANCE:
/* backtracking into clause/2 */
StoreH;
goto r_retry_instance;
w_get_constraint:
case GET_CONSTRAINT:
t1 = Xb(P1);
LoadCVA(t2,H);
SwitchOnVar(t1,t0,
      {BindHVA(t1,t2); Xb(P1)=t2;},
      {BindCVA(t2,t1); Wake;},
      {BindSVA(t1,t2); Xb(P1)=t2;},
      {BindCVA(t2,t1); Wake;});
DISPATCH_W(1);
w_unify_void:
case UNIFY_VOID:
for (i=SP1, P++; i>4; --i) ConstrHVA(H);
goto w_unify_void_4;
w_unify_void_4:
case UNIFY_VOID_4:
ConstrHVA(H);
goto w_unify_void_3;
w_unify_void_3:
case UNIFY_VOID_3:
ConstrHVA(H);
goto w_unify_void_2;
w_unify_void_2:
case UNIFY_VOID_2:
ConstrHVA(H);
goto w_unify_void_1;
w_unify_void_1:
case UNIFY_VOID_1:
ConstrHVA(H);
DISPATCH_W(0);
w_unify_x_variable:
case UNIFY_X_VARIABLE:
U1_XVAR_W(P1);
DISPATCH_W(1);
w_unify_x_value:
case UNIFY_X_VALUE:
U1_XVAL_W(P1);
DISPATCH_W(1);
w_unify_x_local_value:
case UNIFY_X_LOCAL_VALUE:
U1_XLVAL_W(P1);
DISPATCH_W(1);
w_unify_y_first_variable:
case UNIFY_Y_FIRST_VARIABLE:
ComputeE;
goto w_unify_y_variable;
w_unify_y_variable:
case UNIFY_Y_VARIABLE:
U1_YVAR_W(P1);
DISPATCH_W(1);
w_unify_y_first_value:
case UNIFY_Y_FIRST_VALUE:
U1_YFVAL_W(P1);
DISPATCH_W(1);
w_unify_y_value:
case UNIFY_Y_VALUE:
U1_YVAL_W(P1);
DISPATCH_W(1);
w_unify_y_local_value:
case UNIFY_Y_LOCAL_VALUE:
U1_YLVAL_W(P1);
DISPATCH_W(1);
w_unify_constantq:
case UNIFY_CONSTANTQ:
HeapPush(H,T2);
DISPATCH_W(1+BPTP);
w_unify_constant:
case UNIFY_CONSTANT:
HeapPush(H,T1);
DISPATCH_W(0+BPTP);
w_unify_largeq:
case UNIFY_LARGEQ:
P++;
goto w_unify_large;
w_unify_large:
case UNIFY_LARGE:
w->global_top = HeapOffset(H,1);
*H = MakeLarge(Arg,Pplus1);
DISPATCH_R(0+LargeInsns(T1));
w_unify_structureq:
case UNIFY_STRUCTUREQ:
HeapPush(H,Tag(STR,HeapOffset(H,1)));
HeapPush(H,T2);
DISPATCH_W(1+BPTP);
w_unify_structure:
case UNIFY_STRUCTURE:
HeapPush(H,Tag(STR,HeapOffset(H,1)));
HeapPush(H,T1);
DISPATCH_W(0+BPTP);
w_unify_nil:
case UNIFY_NIL:
HeapPush(H,atom_nil);
DISPATCH_W(0);
w_unify_list:
case UNIFY_LIST:
HeapPush(H,Tag(LST,HeapOffset(H,1)));
DISPATCH_W(0);
w_unify_constant_neck_proceedq:
case UNIFY_CONSTANT_NECK_PROCEEDQ:
HeapPush(H,T2);
goto w_neck_proceed;
w_unify_constant_neck_proceed:
case UNIFY_CONSTANT_NECK_PROCEED:
HeapPush(H,T1);
goto w_neck_proceed;
w_unify_nil_neck_proceed:
case UNIFY_NIL_NECK_PROCEED:
HeapPush(H,atom_nil);
goto w_neck_proceed;
w_u2_void_xvar:
case U2_VOID_XVAR:
U1_VOID_W(SP1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_void_yfvar:
case U2_VOID_YFVAR:
ComputeE;
goto w_u2_void_yvar;
w_u2_void_yvar:
case U2_VOID_YVAR:
U1_VOID_W(SP1);
U1_YVAR_W(P2);
DISPATCH_W(2);
w_u2_void_xval:
case U2_VOID_XVAL:
U1_VOID_W(SP1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_void_xlval:
case U2_VOID_XLVAL:
U1_VOID_W(SP1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_void_yfval:
case U2_VOID_YFVAL:
U1_VOID_W(SP1);
U1_YFVAL_W(P2);
DISPATCH_W(2);
w_u2_void_yval:
case U2_VOID_YVAL:
U1_VOID_W(SP1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_void_ylval:
case U2_VOID_YLVAL:
U1_VOID_W(SP1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_u2_xvar_void:
case U2_XVAR_VOID:
U1_XVAR_W(P1);
U1_VOID_W(SP2);
DISPATCH_W(2);
w_u2_xvar_xvar:
case U2_XVAR_XVAR:
U1_XVAR_W(P1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_xvar_yfvar:
case U2_XVAR_YFVAR:
ComputeE;
goto w_u2_xvar_yvar;
w_u2_xvar_yvar:
case U2_XVAR_YVAR:
U1_XVAR_W(P1);
U1_YVAR_W(P2);
DISPATCH_W(2);
w_u2_xvar_xval:
case U2_XVAR_XVAL:
U1_XVAR_W(P1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_xvar_xlval:
case U2_XVAR_XLVAL:
U1_XVAR_W(P1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_xvar_yfval:
case U2_XVAR_YFVAL:
U1_XVAR_W(P1);
U1_YFVAL_W(P2);
DISPATCH_W(2);
w_u2_xvar_yval:
case U2_XVAR_YVAL:
U1_XVAR_W(P1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_xvar_ylval:
case U2_XVAR_YLVAL:
U1_XVAR_W(P1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_u2_yfvar_void:
case U2_YFVAR_VOID:
ComputeE;
goto w_u2_yvar_void;
w_u2_yvar_void:
case U2_YVAR_VOID:
U1_YVAR_W(P1);
U1_VOID_W(SP2);
DISPATCH_W(2);
w_u2_yfvar_xvar:
case U2_YFVAR_XVAR:
ComputeE;
goto w_u2_yvar_xvar;
w_u2_yvar_xvar:
case U2_YVAR_XVAR:
U1_YVAR_W(P1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_yfvar_yvar:
case U2_YFVAR_YVAR:
ComputeE;
goto w_u2_yvar_yvar;
w_u2_yvar_yvar:
case U2_YVAR_YVAR:
U1_YVAR_W(P1);
U1_YVAR_W(P2);
DISPATCH_W(2);
w_u2_yfvar_xval:
case U2_YFVAR_XVAL:
ComputeE;
goto w_u2_yvar_xval;
w_u2_yvar_xval:
case U2_YVAR_XVAL:
U1_YVAR_W(P1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_yfvar_xlval:
case U2_YFVAR_XLVAL:
ComputeE;
goto w_u2_yvar_xlval;
w_u2_yvar_xlval:
case U2_YVAR_XLVAL:
U1_YVAR_W(P1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_yfvar_yval:
case U2_YFVAR_YVAL:
ComputeE;
goto w_u2_yvar_yval;
w_u2_yvar_yval:
case U2_YVAR_YVAL:
U1_YVAR_W(P1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_yfvar_ylval:
case U2_YFVAR_YLVAL:
ComputeE;
goto w_u2_yvar_ylval;
w_u2_yvar_ylval:
case U2_YVAR_YLVAL:
U1_YVAR_W(P1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_u2_yfval_void:
case U2_YFVAL_VOID:
U1_YFVAL_W(P1);
U1_VOID_W(SP2);
DISPATCH_W(2);
w_u2_yfval_xvar:
case U2_YFVAL_XVAR:
U1_YFVAL_W(P1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_yfval_yfval:
case U2_YFVAL_YFVAL:
U1_YFVAL_W(P1);
U1_YFVAL_W(P2);
DISPATCH_W(2);
w_u2_yfval_xval:
case U2_YFVAL_XVAL:
U1_YFVAL_W(P1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_yfval_xlval:
case U2_YFVAL_XLVAL:
U1_YFVAL_W(P1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_yfval_yval:
case U2_YFVAL_YVAL:
U1_YFVAL_W(P1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_yfval_ylval:
case U2_YFVAL_YLVAL:
U1_YFVAL_W(P1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_u2_xval_void:
case U2_XVAL_VOID:
U1_XVAL_W(P1);
U1_VOID_W(SP2);
DISPATCH_W(2);
w_u2_xlval_void:
case U2_XLVAL_VOID:
U1_XLVAL_W(P1);
U1_VOID_W(SP2);
DISPATCH_W(2);
w_u2_xval_xvar:
case U2_XVAL_XVAR:
U1_XVAL_W(P1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_xlval_xvar:
case U2_XLVAL_XVAR:
U1_XLVAL_W(P1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_xval_yfvar:
case U2_XVAL_YFVAR:
ComputeE;
goto w_u2_xval_yvar;
w_u2_xval_yvar:
case U2_XVAL_YVAR:
U1_XVAL_W(P1);
U1_YVAR_W(P2);
DISPATCH_W(2);
w_u2_xlval_yfvar:
case U2_XLVAL_YFVAR:
ComputeE;
goto w_u2_xlval_yvar;
w_u2_xlval_yvar:
case U2_XLVAL_YVAR:
U1_XLVAL_W(P1);
U1_YVAR_W(P2);
DISPATCH_W(2);
w_u2_xval_xval:
case U2_XVAL_XVAL:
U1_XVAL_W(P1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_xval_xlval:
case U2_XVAL_XLVAL:
U1_XVAL_W(P1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_xlval_xval:
case U2_XLVAL_XVAL:
U1_XLVAL_W(P1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_xlval_xlval:
case U2_XLVAL_XLVAL:
U1_XLVAL_W(P1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_xval_yfval:
case U2_XVAL_YFVAL:
U1_XVAL_W(P1);
U1_YFVAL_W(P2);
DISPATCH_W(2);
w_u2_xlval_yfval:
case U2_XLVAL_YFVAL:
U1_XLVAL_W(P1);
U1_YFVAL_W(P2);
DISPATCH_W(2);
w_u2_xval_yval:
case U2_XVAL_YVAL:
U1_XVAL_W(P1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_xval_ylval:
case U2_XVAL_YLVAL:
U1_XVAL_W(P1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_u2_xlval_yval:
case U2_XLVAL_YVAL:
U1_XLVAL_W(P1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_xlval_ylval:
case U2_XLVAL_YLVAL:
U1_XLVAL_W(P1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_u2_yval_void:
case U2_YVAL_VOID:
U1_YVAL_W(P1);
U1_VOID_W(SP2);
DISPATCH_W(2);
w_u2_ylval_void:
case U2_YLVAL_VOID:
U1_YLVAL_W(P1);
U1_VOID_W(SP2);
DISPATCH_W(2);
w_u2_yval_xvar:
case U2_YVAL_XVAR:
U1_YVAL_W(P1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_ylval_xvar:
case U2_YLVAL_XVAR:
U1_YLVAL_W(P1);
U1_XVAR_W(P2);
DISPATCH_W(2);
w_u2_yval_yvar:
case U2_YVAL_YVAR:
U1_YVAL_W(P1);
U1_YVAR_W(P2);
DISPATCH_W(2);
w_u2_ylval_yvar:
case U2_YLVAL_YVAR:
U1_YLVAL_W(P1);
U1_YVAR_W(P2);
DISPATCH_W(2);
w_u2_yval_yfval:
case U2_YVAL_YFVAL:
U1_YVAL_W(P1);
U1_YFVAL_W(P2);
DISPATCH_W(2);
w_u2_ylval_yfval:
case U2_YLVAL_YFVAL:
U1_YLVAL_W(P1);
U1_YFVAL_W(P2);
DISPATCH_W(2);
w_u2_yval_xval:
case U2_YVAL_XVAL:
U1_YVAL_W(P1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_yval_xlval:
case U2_YVAL_XLVAL:
U1_YVAL_W(P1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_ylval_xval:
case U2_YLVAL_XVAL:
U1_YLVAL_W(P1);
U1_XVAL_W(P2);
DISPATCH_W(2);
w_u2_ylval_xlval:
case U2_YLVAL_XLVAL:
U1_YLVAL_W(P1);
U1_XLVAL_W(P2);
DISPATCH_W(2);
w_u2_yval_yval:
case U2_YVAL_YVAL:
U1_YVAL_W(P1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_yval_ylval:
case U2_YVAL_YLVAL:
U1_YVAL_W(P1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_u2_ylval_yval:
case U2_YLVAL_YVAL:
U1_YLVAL_W(P1);
U1_YVAL_W(P2);
DISPATCH_W(2);
w_u2_ylval_ylval:
case U2_YLVAL_YLVAL:
U1_YLVAL_W(P1);
U1_YLVAL_W(P2);
DISPATCH_W(2);
w_bump_counterq:
case BUMP_COUNTERQ:
P++;
goto w_bump_counter;
w_bump_counter:
case BUMP_COUNTER:
#if GAUGE
  INCR_COUNTER(*(ENG_INT **)P);
#endif
DISPATCH_W(BPL);
w_counted_neckq:
case COUNTED_NECKQ:
P++;
goto w_counted_neck;
w_counted_neck:
case COUNTED_NECK:
CODE_COUNTED_NECK;
P += 4;
goto w_neck;
w_fail:
case FAIL:
goto fail;
w_heapmargin_callq:
case HEAPMARGIN_CALLQ:
P++;
goto w_heapmargin_call;
w_heapmargin_call:
case HEAPMARGIN_CALL:
if (HeapDifference(H,Heap_End) < LP1) {
  StoreH;
  explicit_heap_overflow(Arg,LP1, SP3);
  LoadH;
  t0 = X(0);		/* if followed by get_*_x0 */
}
DISPATCH_W(1+BPL);
w_neck:
case NECK:
CODE_NECK;
DISPATCH_W(0);
w_dynamic_neck_proceed:
case DYNAMIC_NECK_PROCEED:
Unify_atom_internal(PointerToTerm(ins),X(3));
if (!w->next_alt)
  goto proceed_w;
SetB(w->node);
if (!B->next_alt && (def_clock = use_clock+1)==0xffff) {
  StoreH;
  clock_overflow(Arg);
  LoadH;
}
goto w_neck_proceed;
w_neck_proceed:
case NECK_PROCEED:
CODE_NECK_PROCEED;
w_proceed:
case PROCEED:
proceed_w:
CODE_PROCEED;
DISPATCH_W(0);
