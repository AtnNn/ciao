...
// #digits + #letters
double invlog2[] = {
                   0, (double)(0x7FF0000000000000LL),      1, 0.6309297535714574,                0.5,
...
/*
  this line let to generate digits for any base up to 36.  The last two digits in the digits array,
  represents the exponent identifier, and the fractional separator.
*/
char digits_upper[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZe.";
char digits_lower[] = "0123456789abcdefghijklmnopqrstuvwxyzE.";

void fillpowtable(int B) {
  int i;
  int m = (int)floor(MAX_EXP*invlog2[B]);
  int n = (int)ceil(MIN_EXP*invlog2[B]);

  powtable[B] = (long double *)malloc((n+1)*sizeof(long double));
  for(i=0;i<=n;i++) {
    (powtable[B])[i] = powl_int(B, i);
  }

  invpowtable[B] = (long double *)malloc((m+1)*sizeof(long double));
  for(i=0;i<=m;i++) {
    (invpowtable[B])[i] = powl_int(B, -i);
  }
};
