...
segmentation violation problem which appeared when backtracking over
concurrent predicates.  The solution was substituting
...
ciao_free() and ciao_malloc(), now only interfaces to free and malloc
(MCL)").

:- comment(version(1*9+48,2003/01/07,14:27*09+'CET'), "Added support
to test conversion of a Ciao integer into a machine int, and to make
numeric conversions through character strings.
