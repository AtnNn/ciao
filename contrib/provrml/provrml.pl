...
:- comment(copyright,"
Copyright @copyright{} 1998-2001 The Clip Group.

@include{Copyright.Manuals}
").

:- comment(module,"@apl{ProVRML} is Prolog library to handle VRML code.
...
:- use_module(library('provrml/io'),
        [read_terms_file/2,
         ...
         write_terms_file/2,
         write_vrml_file/2]).
:- use_module(library('provrml/parser'),
        [parser/2]).
...
:- comment(version(0*1+1,1998/12/10,16:19*45+'MET'), "Commenting.  (Goran
   Smedback)").

:- comment(version(0*1+0,1998/12/02), "First version.
   (Goran Smedback)").

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %% :- comment(module, "This file implements various predicates involving VRML.
 %%      
 %%     
