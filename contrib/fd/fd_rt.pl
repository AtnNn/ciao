...
:- comment(summary, "This library provides a means to write Constraint Logic 
 Programming programs over Finite Domains (@em{CLP(FD)}) and offers a 
 predicate @pred{labeling/1} that implements the labeling process."). 
...
process_propagation_const(min_div_max(X, Y, Z),_,_) :- !,
	...
	Infimus =< X.
process_propagation_const(A,_,_) :- display(unserved_ppc(A)), nl.
...
:- pred labeling(Vars) : list # "Implements the labeling
   ...
   the predicate returns all possible assignments.".
...
:- pred choose_var(+ListOfVars, -Var, -RestOfVars) : list * var * list
   # "Returns a variable @var{Var} from a list of variables
   @var{ListOfVars} and the rest of the list @var{RestOfVars}in a
   deterministic way. Currently it always returns the first variable of
   the list.".

choose_var([X|Xs], X, Xs).

:- pred choose_free_var(+ListOfVars, -Var) : list * var
   # "Returns a free variable @var{Var} from a list of variables 
   @var{ListOfVars}. Currently it always returns the first free variable of 
   the list.". 
...
:- pred choose_var_nd(+ListOfVars, -Var, -RestOfVars) : list * var * list 
   # "Returns a variable @var{Var} from a list of variables @var{ListOfVars} 
   and  the rest of the list @var{RestOfVars}in a non deterministic way.". 


choose_var_nd(L, X) :-
	member(X, L).

:- pred choose_value(+Var, -Value) : var * int
   # "As output produces a value @var{Value} from the domain of @var{Var}. On 
