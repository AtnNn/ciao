/*
  Redefiner for statistics

  Implemented by Edison Mera.

Motivation:

Implementing correct time benchmarking for profiling tools.
===========================================================
...
1. Be independent of the plattform speed.  That is, the objective ot
the benchmark is to measure the time of execution of any piece of
software independently of the plattform used to do the proof.

2. Do use of the high capacity and speed of modern computers, by this
way, at most power computing, one could espect better results, and for
the same benckmark, the improvement of the hardware platform must be
reflected in a improvement of the performance of the profiling tool.
...
4GHz computer, then the task will take 0.25 seconds.  But there are
some issues: modern computers could execute more than 1 instruction by
clock cycle, some laptops could slow down your cpu clock to improve
power managment, and there are a few of optimizations such as the
unsorted excecution, that are the benchmarks imprecise, and for that,
make completely hardware independent test is a chalenge task.
 ...
 */
#include <hrtime.h>
#include "../../../engine/datadefs.h"
#include "../../../engine/timing_defs.h"
...
long init_statistics_hrtime(void) {
  long r;
