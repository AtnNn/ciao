:- module(component_registry_base,
...
:- doc(title, "Registry of Ciao Components (base)").
:- doc(author, "Edison Mera (original author)").
:- doc(author, "Jose F. Morales").

component_setup_dir(all, SetupDir) :-
	absolute_file_name(library(component_registry), '_opt', '.pl', '.', _, _,
	    RootDir),
...
component_setup_dir(all, RealLibDir, SetupDir) :-
	atom_concat(RealLibDir, '/lib/component_registry/components/', SetupDir).
