...
%	[assertions,hiord,isomodes,regtypes,foreign_interface]).
...
:- true pred new_ccall(+Sid, +NewSid, +Bindings, -F, ?E) ::
        id * id * bindings * cont * dummy 
...
%:- extra_compiler_opts(['-g -DDEBUG']).


:- use_foreign_source(cont_callsPADL).
