...
static TrNode put_trie(struct memo_register *memo, TrNode node, ENGINE_Term entry);
...
static inline
TrNode trie_node_check_insert(struct memo_register *memo, TrNode parent, ENGINE_Term t) {
  TrNode child;
...
TrNode open_trie(struct memo_register *memo) 
{
  TrNode new_node;
...
TrNode put_trie_entry(struct memo_register *memo, TrNode node, ENGINE_Term entry) 
{
  stack_args_base = stack_args = TERM_STACK;
  stack_vars_base = stack_vars = TERM_STACK + TERM_STACK_SIZE - 1;

  node = put_trie(memo, node, entry);
...
static
TrNode put_trie(struct memo_register *memo, TrNode node, ENGINE_Term entry) 
{
  ENGINE_Term t;
  DEREF(t,entry);
  if (IsTrieVar(t)) {
    node = trie_node_check_insert(memo,node, t);
  } else if (ENGINE_IsVarTerm(t)) {
    node = trie_node_check_insert(memo,node, ENGINE_VarTrie | ((stack_vars_base - stack_vars) << 1));
    ...
    if (TagIsSmall(t)) node = trie_node_check_insert(memo,node, t);
    else  //large integer
      {
	node = trie_node_check_insert(memo,node, ENGINE_LargeInitTag);
	node = trie_node_check_insert(memo,node, *(TagToPointer(t) + 1));
	node = trie_node_check_insert(memo,node, ENGINE_LargeEndTag);
      }
  } else if (ENGINE_IsFloatTerm(t)) {
	node = trie_node_check_insert(memo,node, ENGINE_FloatInitTag);
	node = trie_node_check_insert(memo,node, *(TagToPointer(t) + 1));
	node = trie_node_check_insert(memo,node, *(TagToPointer(t) + 2));
	node = trie_node_check_insert(memo,node, ENGINE_FloatEndTag);
  } else if (ENGINE_IsPairTerm(t)) {
    node = trie_node_check_insert(memo,node, ENGINE_PairInitTag);
    do 
      {
	node = put_trie(memo, node, ENGINE_HeadOfTerm(t));
	DEREF(t,ENGINE_TailOfTerm(t));
      } while (ENGINE_IsPairTerm(t));
    node = put_trie(memo, node, t);
    node = trie_node_check_insert(memo, node, ENGINE_PairEndTag);
  } else if (ENGINE_IsApplTerm(t)) {
    if (!strcmp(ENGINE_NameOfFunctor(t),",") && ENGINE_ArityOfFunctor(t)  == 2) 
      {
	node = trie_node_check_insert(memo, node, ENGINE_CommaInitTag);
	do 
	  {
	    node = put_trie(memo, node, ENGINE_ArgOfTerm(1, t));
	    DEREF(t,ENGINE_ArgOfTerm(2, t));
	  } while (ENGINE_IsApplTerm(t) && !strcmp(ENGINE_NameOfFunctor(t),",") && ENGINE_ArityOfFunctor(t)  == 2);
	node = put_trie(memo, node, t);
	node = trie_node_check_insert(memo, node, ENGINE_CommaEndTag);
      } 
    else 
      {
	int i;
	node = trie_node_check_insert(memo, node, CTagToPointer(t));
	for (i = 1; i <= ENGINE_ArityOfFunctor(t); i++)
	  {
	    node = put_trie(memo, node, ENGINE_ArgOfTerm(i, t));
	  }
      }
...
ENGINE_Term get_trie_entry(worker_t *Arg, TrNode node) 
{
