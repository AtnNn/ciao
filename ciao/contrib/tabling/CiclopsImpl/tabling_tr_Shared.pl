	...
	( 'trans$tab'(P,A) ->
	  ...
	  ; ( 'trans$tab'(P,A) ->
	    ...
	    ; assert('trans$tab'(P,A)),
	      ...
	      Clauses = [(Head :- 
			 tabled_call(Head,Id,PredInit,'true',CCall),
			 (
			     nonvar(CCall) ->
			     ('$meta_call'(CCall);test_complete(Id))
			 ;
			     true
			 ),
			 is_lider(Id),
			 consume_answer(Head,Id,_)
			 )|_Cls],
...
expand_command_table_one(Pspec,Clauses0,Clauses) :-
	  ...
	  ; assert('trans$tab'(P,A)),
	    ...
	    Clauses0 = [
            (H :- tabled_call(H,Id,PredInit,'true',CCall),
	          (
		      nonvar(CCall) -> 
		      ('$meta_call'(CCall);test_complete(Id))
		  ;
...
get_transcall(Blist,HeadVars,_Delay,Cid,TRANSCall,Cont,_Delay1,ListVars,Diff) :-
	...
	TRANSCall = (tabled_call(Call,Sid,PredInit,ContPredMod,CCall),
		    (
			nonvar(CCall) ->
			('$meta_call'(CCall); test_complete(Sid))
		    ;
			true
		    ),
		    read_answers(Cid,Sid,Diff,F,_,_),
		    '$meta_call'(F)),
...
%	    Cls0 = [(:- 'trans$prolog'(Pred) -> true; assert('trans$prolog'(Pred)))|Cls1],
	  ...
	  ; 'trans$default'((tabled)) ->
	    ...
	    Cls0 = [(:- retractall('trans$tabled'(P,A)), assert('trans$tabled'(P,A))),
                    (Head :- tabled_call(Head,Id,PredInit,'true',CCall),
		             (
				 nonvar(CCall) -> 
				 ('$meta_call'(CCall);test_complete(Id))
			     ;
