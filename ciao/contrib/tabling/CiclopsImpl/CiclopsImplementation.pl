...
:- load_compilation_module(library(tabling('CiclopsImplementation'(tabling_tr_Shared)))).

:- add_sentence_trans(do_term_expansion/3).

:- use_module(library(tabling('CiclopsImplementation'(tabling_rt_Shared)))).
